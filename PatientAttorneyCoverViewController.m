...
- (IBAction)cancel:(id)sender {
    [self performSegueWithIdentifier:@"attorneytowelcome" sender:self];
}
...
-(void)Getdata
{
    ...
    NSArray *items1App=[itemsApp objectForKey:@"patientattorneyuser List"];
    ...
    if ([items1App count]>0)
    {
        ...
        for (id anUpdate1 in items1App)
        {
            ...
            temp4 =[arrayList1 objectForKey:@"date"];
            temp5 =[arrayList1 objectForKey:@"dearsir"];
            temp6=[arrayList1 objectForKey:@"nameofclinic"];
            temp7=[arrayList1 objectForKey:@"treat"];
...
-(void)updatedata
{
    ...
    else
    {
        ...
        if ([[menu objectForKey:@"servicename"] isEqualToString:@"patientattorney Data"])
        {
...
-(NSString *)HttpPostEntityFirst1:(NSString*)firstEntity ForValue1:(NSString*)value1 EntitySecond:(NSString*)secondEntity ForValue2:(NSString*)value2
{
    ...
    NSString *url1=@"Patientattorneycoverletter.php?service=patientattorneyinsert";
    
    NSString *url2=[NSString stringWithFormat:@"%@%@",urltemp,url1];
    NSString *post =[[NSString alloc] initWithFormat:@"%@=%@&name=%@&address=%@&reg=%@&patientname=%@&date=%@&dearsir=%@&nameofclinic=%@&treat=%@&%@=%@",firstEntity,value1,patattory.text,addrs.text,reg.text,patname.text,dofacc.text,dearname.text,sincname.text,physicianname.text,secondEntity,value2];
...
-(NSString *)HttpPostEntityFirst1update:(NSString*)firstEntity ForValue1:(NSString*)value1 EntitySecond:(NSString*)secondEntity ForValue2:(NSString*)value2
{
    ...
    NSString *urltemp=[[databaseurl sharedInstance]DBurl];
    NSString *url1=@"Patientattorneycoverletter.php?service=patientattorneyedit";
...
-(NSString *)HttpPostEntityFirst1delete:(NSString*)firstEntity ForValue1:(NSString*)value1 EntitySecond:(NSString*)secondEntity ForValue2:(NSString*)value2
{
    ...
    NSString *urltemp=[[databaseurl sharedInstance]DBurl];
    NSString *url1=@"Patientattorneycoverletter.php?service=patientattorneydelete";
...
-(NSString *)HttpPostGetdetails:(NSString*)firstEntity ForValue1:(NSString*)value1 EntitySecond:(NSString*)secondEntity ForValue2:(NSString*)value2
{
    ...
    NSString *urltemp=[[databaseurl sharedInstance]DBurl];
    NSString *url1=@"Patientattorneycoverletter.php?service=patientattorneyselect";
...
- (IBAction)submit:(id)sender {
    ...
    if([patattory.text length]!=0&&[reg.text length]!=0&&[addrs.text length]!=0&&[patname.text length]!=0&&[dofacc.text length]!=0&&[dearname.text length]!=0&&[sincname.text length]!=0&&[physicianname.text length]!=0){
        if([patattory.text length]==0||([self validateNames:texty1]==1))
        {
            ...
            {
                if([addrs.text length]==0||([self validateAdds:texty3]==1))
                {
                        ...
                        if([dofacc.text length]==0||([self validateDate:texty5]==1))
                        {
                            if([dearname.text length]==0||([self validateNames:texty6]==1))
                            {
                                if([sincname.text length]==0||([self validateNames:texty7]==1))
                                {
                                    ...
                                    [[TWMessageBarManager sharedInstance] showMessageWithTitle:kStringMessageBarErrorTitle
                                                                                   description:@"Enter valid signature name."
                                                                                          type:TWMessageBarMessageTypeError
                                                                                statusBarStyle:UIStatusBarStyleLightContent
                                                                                      callback:nil];
                                }
